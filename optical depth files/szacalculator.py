...
class SZA:
    ...
    def Sza(self,lat,long,hour,minute):
        dobj = datetime.datetime(2022,2,self.date,hour,minute,tzinfo=datetime.timezone.utc)
...
class Trans:
    ...
    def __init__(self,opdepth,secZ):
        self.opdepth = opdepth 
        self.secZ = secZ
        
        
    def tau(self): 
        t =[]
        for i in range(len(self.secZ)):   
            T = np.exp(-self.opdepth*self.secZ[i])
            t.append(T)
        return t
        
    def plot(self):
        plt.plot(self.secZ,self.tau(),'x')
